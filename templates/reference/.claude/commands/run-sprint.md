...
Invoque le subagent `scrum-master`. Trois issues possibles :
...
2. **Lead-dev** — s'il y a des tâches `status == "todo"` et pas encore assignées, invoque `lead-dev` pour qu'il les découpe et les assigne.

3. **Exécution** — pour chaque tâche en `status == "in_progress"`, invoque l'agent correspondant à son `assignee` :
   ...
   Tu peux invoquer plusieurs exécutants en parallèle si leurs tâches sont indépendantes (pas de dépendances croisées dans `dependencies`).

4. **Review** — pour chaque tâche en `status == "in_review"` avec `assignee == "code-reviewer"`, invoque `code-reviewer`.

4b. **Security Review** — en parallèle de l'étape 4, invoque `security-reviewer` sur toutes les tâches en `status == "in_review"`. Si un finding CRITIQUE est détecté, la tâche revient en `in_progress` (prime sur la décision du code-reviewer).

5. **QA** — pour chaque tâche en `status == "qa"` avec `assignee == "qa-tester"`, invoque `qa-tester`.
